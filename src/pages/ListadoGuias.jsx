...
import { getAllGuias } from '../peticiones/guias_peticiones.mjs';
...
function Guias() {
  ...
  const guiasFiltradas = guias.filter((guia) => {
    ...
    return (matchesUser || matchesTitle) && matchesGame;
    });

  const sortedGuias = [...guiasFiltradas];

  if (orderBy === 'asc') {
    // Ordenar por título de la guía (A-Z)
    sortedGuias.sort((a, b) => a.contenido.titulo.localeCompare(b.contenido.titulo));
  } else if (orderBy === 'desc') {
    // Ordenar por título de la guía (Z-A)
